...
class ProcessReturn
{
    ...
    private function route(OrderInterface $order, PaymentResponse $response, bool $isProven): string
    {
        ...
        if ($isProven) {
            $this->logger->info(sprintf(
                'Ogloba order %s - the payment return reports %s, the cart is restored and the order is left '
        ...
        $this->logger->info(sprintf(
            'Ogloba order %s - the payment return reports %s without proving it, the customer is sent to the '
            . 'confirmation page and the order is left for the notification to resolve.',
            $incrementId,
            $response->getOrderStatus()
        ));

        return OutcomeInterface::UNCONFIRMED;
    }
    ...
    private function isProven(OrderInterface $order, PaymentResponse $response): bool
    {
        ...
        if ($this->isPaymentResponseSigned->execute($response, (int) $order->getStoreId()) === false) {
            $this->logger->error(sprintf(
                'Ogloba order %s - invalid signature on the payment return received from %s, '
                . 'payload: %s (sign: %s)',
                $incrementId,
                (string) $this->remoteAddress->getRemoteAddress(),
                $this->getReceivedPayload($response),
    ...
    /**
     * Get the payload exactly as it was received, so a return Ogloba did not sign can be examined afterwards
     *
     * @param PaymentResponse $response
     * @return string
     */
    private function getReceivedPayload(PaymentResponse $response): string
    {
        $encoded = $response->getEncodedResponse();

        return $encoded !== '' ? $encoded : $this->json->serialize($response->getResponse());
    }
}
